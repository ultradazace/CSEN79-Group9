...
namespace ebay
{
    ...
    public:
        ...
        int getHighestBid() const { return highestBid; }
        std::string getWinner() { return winner; };
        std::vector<Bid> getBids() const { return bids; };

        void addBid(const std::string userName, const double bidAmount); // This is to add and check if b is greater than the highest bid
    ...
    private:
        ...
        int highestBid;
        std::vector<Bid> bids; // The current bids for the item
    };
